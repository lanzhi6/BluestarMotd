...
public final class v1_19_R1 extends VersionManager
{
    ...
    {
        v1_19_R1.infoGetter=infoGetter;
    ...
    private ServerPing parseServerPing()
    {
        ...
        ping.a(CraftChatMessage.fromString(info.getMotd(),true)[0]);
        CachedServerIcon icon=info.getIcon();
        if (icon instanceof CraftIconCache)
        {
            ping.a(((CraftIconCache) icon).value);
        }
        else if (Bukkit.getServerIcon() instanceof CraftIconCache)
        {
            ping.a(((CraftIconCache) Bukkit.getServerIcon()).value);
        }
        ...
        ping.a(playerSample);
        String serverName=MinecraftServer.getServer().getServerModName()+" "+SharedConstants.b().getName();
        ping.a(new ServerPing.ServerData(info.getServerName()!=null?info.getServerName():serverName,
                                         info.getServerVersion()?-1:SharedConstants.b().getProtocolVersion()));
        return ping;
    }
}
